...
import { aggregateMetrics, formatDelta, percentChange, type DailyMetric } from "@/lib/reporting";
...
export function ReportDocument({ report, client, currentRows, previousRows, topQueries, topPages }: { report: ReportRecord; client: { name: string; domain: string }; currentRows: DailyMetric[]; previousRows: DailyMetric[]; topQueries: DimensionRow[]; topPages: DimensionRow[] }) {
  const current = aggregateMetrics(currentRows); const previous = aggregateMetrics(previousRows);
  const cards = [
    ["Clicks", current.clicks.toLocaleString("en-GB"), percentChange(current.clicks, previous.clicks)],
  ...
  return <article className="report-document mx-auto max-w-6xl bg-[#f4f1e9] text-[#181818]">
    <header className="rounded-[2rem] bg-[#181818] p-8 text-white sm:p-12"><Image src="/search-society-logo.png" alt="Search Society" width={960} height={192} className="h-9 w-auto brightness-0 invert" /><p className="mt-12 text-sm font-bold uppercase tracking-[0.22em] text-lime-400">Organic search performance</p><h1 className="mt-3 max-w-4xl text-4xl font-bold tracking-tight sm:text-6xl">{report.title}</h1><div className="mt-8 flex flex-wrap gap-x-8 gap-y-2 text-sm text-slate-300"><span>{client.name}</span><span>{client.domain}</span><span>{period}</span></div></header>
    <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{cards.map(([label, value, delta]) => <div key={label} className="rounded-2xl bg-white p-5 shadow-sm"><p className="text-sm font-semibold text-slate-500">{label}</p><p className="mt-3 text-3xl font-bold">{value}</p><p className={`mt-2 text-xs font-bold ${delta !== null && delta < 0 ? "text-rose-600" : "text-emerald-700"}`}>{formatDelta(delta)} vs comparison</p></div>)}</section>
    <section className="mt-8 rounded-3xl bg-white p-6 shadow-sm sm:p-8"><h2 className="text-xl font-bold">Performance trend</h2><p className="mt-1 text-sm text-slate-500">Daily clicks and impressions during the report period.</p><div className="mt-7"><PerformanceChart data={currentRows.map((row) => ({ date: row.metric_date, clicks: Number(row.clicks), impressions: Number(row.impressions) }))} /></div></section>
