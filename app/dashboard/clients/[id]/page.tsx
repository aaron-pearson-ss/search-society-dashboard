  ...
  return (
    <>
      ...
      <div className="mt-5 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        ...
        <div className="flex flex-wrap gap-3"><Link href={`/dashboard/clients/${client.id}/analytics`} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-bold text-slate-800 hover:bg-slate-50"><Icons.reports className="h-4 w-4" />GA4 analytics</Link><Link href={`/dashboard/clients/${client.id}/reports/new`} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-bold text-slate-800 hover:bg-slate-50"><Icons.reports className="h-4 w-4" />Create report</Link>{linkedProperty ? <form action={syncGscPerformance.bind(null, client.id)}><button className="btn-primary" type="submit"><Icons.reports className="h-4 w-4" />Sync GSC data</button></form> : null}<a href={`/api/google/connect?clientId=${client.id}`} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-bold text-slate-800 hover:bg-slate-50"><Icons.globe className="h-4 w-4" />{linkedProperty ? "Reconnect Google" : "Connect Search Console"}</a></div>
      </div>
